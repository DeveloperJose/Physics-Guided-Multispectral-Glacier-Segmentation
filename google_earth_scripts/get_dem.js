...
var elevation = ee.Image('NASA/NASADEM_HGT/001')
      ...
      curvature = ee.Terrain.slope(slope);
// add all the bands to form a single image
var dem = elevation.addBands(slope).addBands(aspect).addBands(curvature);
...
for (var i = 0; i < fishnet.length().getInfo(); i++) {
  ...
  //Map.addLayer(image, {bands: ['elevation'], min:0, max:8850}, 'image');
  //break;
